...
class Bottleneck(nn.Module):
    def __init__(self, in_planes, out_planes, stride, groups):
        ...
        mid_planes = (out_planes//(4*groups))*groups # midplane must be a multiplier of groups
...
class ShuffleNet(nn.Module):
    def __init__(self,
        ...
        if groups == 3:
            ...
            print("Please select group number in [3, 8]")

        out_planes = [int(i * width_mult) for i in out_planes]
        self.conv1 = nn.Conv2d(init_ch, out_planes[0], kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.layer3 = self._make_layer(out_planes[3], num_blocks[3], groups)
        self.linear = nn.Linear(out_planes[3], num_classes)
...
if __name__ == '__main__':
    net = ShuffleNet(num_classes=10, width_mult=1, groups=3, init_ch=3)
